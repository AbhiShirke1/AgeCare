...
app.post('/trial', async (req, res) => {
    ...
    try {
        let date = req.body.setTodaysDate;
...
app.post('/trial2', async (req, res) => {
    ...
    var mailOptions = {
        ...
        subject: 'Age Care',
        text: `Greetings from AgeCare,
        You have succesfully bregistered for the yoga event held at Santacruz on 23rd November, 2022
...
app.post('/ctLogin', async (req, res) => {
    ...
    const ea = await Userprofile.find({ email: cd.useremail });
    let mob = [];
    let addr = [];
    ...
    return res.render('ctDashboard', { name: req.cookies.name, email: cd.useremail, names: cd.usernames, mobile: mob.toString(), address: addr.toString() });
    // }
...
app.post('/finalBook', async (req, res) => {
    ...
    if (a == 0) {
        ...
        res.redirect('https://rzp.io/l/Lv3gNNeAwm')
    }
